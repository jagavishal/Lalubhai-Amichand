...
window.Pages['inward'] = (() => {
  ...
  /* ── Quantity roll-up ─────────────────────────────────────────────────────
     Cancelled entries are left out — their quantity was already reversed back
     off current stock, so counting it would overstate what actually came in.
  ...
  // book's optional Size and Department columns.
  function _leadSpan() { return 3 + (_showSize() ? 1 : 0); }      // Date, Item Code, Description [, Size]
  function _tailSpan() { return 3 + (_showDepartment() ? 1 : 0); } // [Department,] Source, Remarks, Actions
  ...
  /* ── Day-wise grouping ────────────────────────────────────────────────────
     ...
     its own quantity total, worked out from whatever the filters currently
     show. That replaces the raw created_at ordering the list used to have —
     two entries typed on different days no longer interleave just because one
     was keyed in late. ─────────────────────────────────────────────────────── */
