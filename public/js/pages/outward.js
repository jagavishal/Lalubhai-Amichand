...
// Stores tab's Outward form only ever touches the Stores catalog, the Alu & SS
// tab only ALU, and so on (see ims.js). Everything below — the item-code
// typeahead, the "+ New Item" modal, and the Outward List — is scoped to that
// one category.
window.Pages['outward'] = (() => {
  ...
  // Column count of the list table, which gains/loses Issued To and Size
  ...
  // Log/List, or coming back to Outward from another tab — so filling has to
  ...
  // Issued To (Department) dropdown permanently empty from the second render
  // onward.
  ...
  function _bindItemCodeInput(input) {
    ...
    const sizeInput = row.querySelector('.outw-row-size'); // Trading only
    const runSearch = () => {
      ...
      _itemSearchTimer = setTimeout(async () => {
        ...
        try {
          ...
          dd.innerHTML = matches.slice(0, 30).map(m => '<div class="outw-item-opt" style="padding:7px 12px;font-size:12.5px;cursor:pointer;" data-code="' + esc(m.itemCode) + '" data-desc="' + esc(m.description) + '" data-size="' + esc(m.size) + '" data-uom="' + esc(m.uom) + '" data-stock="' + esc(m.currentStock) + '">'
    ...
    dd.addEventListener('mousedown', (e) => {
      ...
      // Prefilled from the catalog but still editable — stock can be issued at
      // a size the catalog row hasn't caught up with yet.
  ...
  function _collectItemRows() {
    return Array.from(document.querySelectorAll('#outw-items-tbody .outw-item-row')).map(row => {
      ...
      const sizeInput = row.querySelector('.outw-row-size'); // absent off the Trading book
      return {
  ...
  // all mount this same module into the same #outwl-body, so without this a
  ...
  async function _load() {
    ...
    let rows = [], err = '';
    try {
      rows = await Utils.apiFetch('/api/ims/outward/list?category=' + encodeURIComponent(_category)) || [];
    } catch (e) {
      err = e.message || 'Failed to load Outward entries';
    }
    ...
    const rowHtml = (r) => ''
      + '<tr style="border-bottom:1px solid #f1f5f9;">'
        ...
        + '<td style="padding:8px 10px;font-size:12.5px;white-space:nowrap;">'
          ...
          + Utils.ownerDeleteBtn('outw-delete-btn', 'id', r.id)
        + '</td>'
      + '</tr>';
  ...
  function _bindRowActions() {
    ...
    body.addEventListener('click', async (e) => {
      const delBtn = e.target.closest('.outw-delete-btn');
      if (delBtn) {
        // A live entry still counts toward current stock, so the server
        // backs its quantity out before removing the row.
        if (!(await Utils.ownerDeleteConfirm('This Outward entry'))) return;
        try {
          await Utils.apiFetch('/api/ims/outward?id=' + encodeURIComponent(delBtn.dataset.id), { method: 'DELETE' });
  ...
  function _listViewHtml() {
    return '<div style="display:flex;align-items:baseline;justify-content:space-between;gap:12px;margin-bottom:14px;flex-wrap:wrap;">'
        + '<p style="font-size:12.5px;color:#64748b;margin:0;">Every stock-out entry logged against the ' + esc(_categoryLabel) + ' catalog.</p>'
