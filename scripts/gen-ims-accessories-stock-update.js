'use strict';
/**
 * Generates a dated MySQL/MariaDB update file that syncs the IMS Accessories
 * book's current_stock to the "Closing Stock" column of the
 * "IMS (Accessory Store)" Google Sheet's IMS tab. Read-only against the
 * sheet — it writes a .sql file for you to review and run against
 * production, it never touches the database itself.
 *
 ...
 *
 *   node scripts/gen-ims-accessories-stock-update.js [--date=YYYY-MM-DD] [--by="Name"] [outfile]
 *
 ...
 *
 * Same ADJ-row approach as scripts/gen-ims-stores-stock-update.js (see that
 * file's header for why a bare UPDATE would corrupt the Report tab's
 * Day-wise Stock history) — this is the Accessories book's twin.
 *
 * Column mapping (IMS tab): A = item code, B = item name, E = Max Level,
 * K = Available in %, L = Closing Stock. Columns M onward are one column per
 * date. This tab carries no UOM column (Accessories items are all 'PCS' —
 * see ims_items_accessories_import.sql), so a brand-new sheet code (STEP 5)
 * is inserted with uom='PCS', matching that original import.
 */
...
const OUT = argv.find(a => !a.startsWith('--'))
  || path.join(__dirname, `../database/imports/ims_accessories_stock_update_${RUN_DATE.replace(/-/g, '')}.sql`);

// Column indexes, 0-based, in the IMS tab's row array.
const IM = { code: 0, name: 1, maxLevel: 4, availablePct: 10, closingStock: 11, firstDateCol: 12 };
...
(async () => {
  ...
  /* ── cross-check the sheet's own Available-% formula before trusting
     Closing Stock — same discipline as the original catalog import. ────── */
  let checkedPct = 0;
  ...
  if (!checkedPct) throw new Error('Could not verify column mapping against any row — the sheet layout has probably changed');
  ...
  for (const r of rows.slice(1)) {
    ...
    items.set(code, { code, name, closing });
  }
  if (stats.mismatch) {
  ...
  console.log(`sheet Closing Stock is for ${sheetDay}; ${list.length} items with a value, ${stats.blank.length} blank, total ${total}, column mapping verified on ${checkedPct} rows`);
  ...
  const tmp = `tmp_accessories_closing_${stamp}`;
  const bak = `ims_items_accessories_stock_bak_${stamp}`;
  ...
  const values = list.map(i => `  (${sqlStr(i.code)}, ${sqlStr(i.name)}, ${i.closing.toFixed(2)})`).join(',\n');

  const out = `-- ============================================================================
-- IMS Accessories -- current_stock sync from the "IMS (Accessory Store)"
-- Google Sheet's Closing Stock column (IMS tab, col L), sheet day ${sheetDay}, run date ${RUN_DATE}
-- ${SHEET_URL}
--
...
--   node scripts/gen-ims-accessories-stock-update.js --date=${RUN_DATE} --by="${CREATED_BY}"
--
-- WHAT THIS FILE DOES
...
--     (${sheetDay}) on every row, and cross-checked Available-% = Closing/Max
--     on ${checkedPct} rows, so this really is that day's closing figure.
...
INSERT INTO ${tmp} (item_code, description, closing) VALUES
${values};
...
SELECT
  ...
  CONCAT('Physical count: ', t.closing + 0, ' (system was ', i.current_stock + 0,
         ...
         '). Synced from IMS (Accessory Store) sheet Closing Stock for ${sheetDay}.'),
  'Active',
  ${sqlStr(CREATED_BY)},
  ''
FROM   ${tmp} t
...
SELECT t.item_code, t.description, '', 'PCS', 0, 0, 0, '', t.closing, ${sqlStr(CATEGORY)}
FROM   ${tmp} t
...
-- STEP 6 -- verification.
-- ----------------------------------------------------------------------------
-- Expect: accessories_items >= ${list.length}, sum_stock = ${total} + stock of the
