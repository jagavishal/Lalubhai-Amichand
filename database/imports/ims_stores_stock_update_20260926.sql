...
-- Closing Stock column (IMS tab, col L), sheet day 2026-09-26, run date 2026-09-26
...
--   node scripts/gen-ims-stores-stock-update.js --date=2026-09-26 --by="Akhilesh (ERP)"
--
-- WHAT THIS FILE DOES
...
--   - Sum of all Closing Stock values loaded = 84210.75.
--   - The generator asserted Closing Stock == the sheet's last date column
--     (2026-09-26) on every row, so this really is that day's closing figure.
...
-- ----------------------------------------------------------------------------
-- CREATE TABLE ims_items_stores_stock_bak_20260926 AS
...
-- UPDATE ims_items i
--   JOIN ims_items_stores_stock_bak_20260926 b ON b.item_code = i.item_code
...
-- ----------------------------------------------------------------------------
DROP TEMPORARY TABLE IF EXISTS tmp_stores_closing_20260926;
CREATE TEMPORARY TABLE tmp_stores_closing_20260926 (
...
INSERT INTO tmp_stores_closing_20260926 (item_code, description, uom, closing) VALUES
  ('SKU 1', 'ALUMINUM WELDING ROD 1.63 MM SMALL', 'Kg', 8.45),
  ('SKU 2', 'ALUMINUM WELDING ROD 2 MM', 'Kg', 13.50),
  ('SKU 3', 'LUBRICANT OIL 40 NO', 'Ltr', 233.22),
  ('SKU 4', 'ALUMINUM DRAWING POWDER', 'kg', 165.10),
  ('SKU 5', 'COTTON WASTE', 'kg', 111.74),
  ('SKU 6', 'WHITE COTTON WASTE', 'kg', 41.45),
  ('SKU 7', 'PHOSPHORIC ACID', 'PCS', 27.00),
  ('SKU 8', 'SULFURIC ACID', 'PCS', 25.00),
  ('SKU 9', 'NITRIC ACID', 'PCS', 22.00),
  ('SKU 10', 'HYDROFLUORIC ACID', 'PCS', 5.00),
  ('SKU 11', 'CHROMIC ACID', 'kg', 62.25),
  ('SKU 12', 'CAUSTIC SODA', 'kg', 300.00),
  ('SKU 13', 'LIME POWDER', 'kg', 0.30),
  ('SKU 15', 'KEROSENE', 'Ltr', 29.11),
  ...
  ('SKU 34', 'POLYETHYLENE BAG 26*26', 'kg', 192.74),
  ...
  ('SKU 62', 'BOPP TAPE ROLL (BROWN)', 'PCS', 247.00),
  ...
  ('SKU 86', 'CONTAINER CARGO NET', 'PCS', 9.00);
...
SELECT
  ...
  CONCAT('Physical count: ', t.closing + 0, ' (system was ', i.current_stock + 0,
         ...
         '). Synced from IMS (Stores) sheet Closing Stock for 2026-09-26.'),
  'Active',
  'Akhilesh (ERP)',
  ''
FROM   tmp_stores_closing_20260926 t
...
-- ----------------------------------------------------------------------------
UPDATE ims_items i
  JOIN tmp_stores_closing_20260926 t ON t.item_code = i.item_code
...
-- STEP 6 -- verification.
-- ----------------------------------------------------------------------------
-- Expect: stores_items >= 84, sum_stock = 84210.75 + stock of the
