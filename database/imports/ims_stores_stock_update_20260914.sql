...
-- Closing Stock column (IMS tab, col L), sheet day 2026-09-14, run date 2026-09-14
...
--   node scripts/gen-ims-stores-stock-update.js --date=2026-09-14 --by="Akhilesh Vyas"
--
-- WHAT THIS FILE DOES
--   STEP 1 (recommended) : backup snapshot of the Stores book's stock, for undo.
--   STEP 2 (required)    : load the sheet's 85 Closing Stock values into a
--                          temporary table.
...
--                          history stays correct for days before 2026-09-14.
...
--
-- NOTES / DATA QUALITY
--   - 87 sheet rows read, 85 carry a Closing Stock, 0 duplicates.
--   - 2 row(s) have a BLANK Closing Stock. Blank is not zero, so they
--     are NOT in the temp table and their DB stock is left untouched:
--       SKU 44 (EXTRA SANDPAPER J-297)
--       SKU 87 (120 NO EMERY PAPER)
--   - 16 of the 85 items close at exactly 0; 0 NEGATIVE.
--   - Sum of all Closing Stock values loaded = 84849.1.
--   - The generator asserted Closing Stock == the sheet's last date column
--     (2026-09-14) on every row, so this really is that day's closing figure.
...
-- ----------------------------------------------------------------------------
-- CREATE TABLE ims_items_stores_stock_bak_20260914 AS
...
-- UPDATE ims_items i
--   JOIN ims_items_stores_stock_bak_20260914 b ON b.item_code = i.item_code
...
-- ----------------------------------------------------------------------------
DROP TEMPORARY TABLE IF EXISTS tmp_stores_closing_20260914;
CREATE TEMPORARY TABLE tmp_stores_closing_20260914 (
...
INSERT INTO tmp_stores_closing_20260914 (item_code, description, uom, closing) VALUES
  ('SKU 1', 'ALUMINUM WELDING ROD 1.63 MM SMALL', 'Kg', 9.45),
  ('SKU 2', 'ALUMINUM WELDING ROD 2 MM', 'Kg', 3.50),
  ('SKU 3', 'LUBRICANT OIL 40 NO', 'Ltr', 33.22),
  ('SKU 4', 'ALUMINUM DRAWING POWDER', 'kg', 13.10),
  ('SKU 5', 'COTTON WASTE', 'kg', 173.24),
  ('SKU 6', 'WHITE COTTON WASTE', 'kg', 45.95),
  ('SKU 7', 'PHOSPHORIC ACID', 'PCS', 5.00),
  ('SKU 8', 'SULFURIC ACID', 'PCS', 38.00),
  ('SKU 9', 'NITRIC ACID', 'PCS', 9.00),
  ('SKU 10', 'HYDROFLUORIC ACID', 'PCS', 5.00),
  ('SKU 11', 'CHROMIC ACID', 'kg', 66.75),
  ('SKU 12', 'CAUSTIC SODA', 'kg', 550.00),
  ('SKU 13', 'LIME POWDER', 'kg', 3.30),
  ('SKU 14', 'COCONUT OIL', 'Ltr', 1.65),
  ('SKU 15', 'KEROSENE', 'Ltr', 39.31),
  ('SKU 16', 'HYDRAULIC OIL 68 NO', 'Ltr', 202.05),
  ('SKU 17', 'COMPRESSOR OIL 220 NO', 'Ltr', 25.00),
  ('SKU 18', 'ALL-PURPOSE GREASE', 'kg', 10.70),
  ('SKU 19', 'BRASS INSERT', 'kg', 45.00),
  ('SKU 20', 'OXYGEN CYLINDER', 'c.m', 3.00),
  ...
  ('SKU 25', 'POLYETHYLENE BAG 11*13', 'kg', 102.95),
  ('SKU 26', 'POLYETHYLENE BAG 12*12', 'kg', 85.14),
  ('SKU 27', 'POLYETHYLENE BAG 13*15', 'kg', 204.40),
  ('SKU 28', 'POLYETHYLENE BAG 14*16', 'kg', 107.85),
  ('SKU 29', 'POLYETHYLENE BAG 16*18', 'kg', 140.50),
  ('SKU 30', 'POLYETHYLENE BAG 18*20', 'kg', 118.18),
  ('SKU 31', 'POLYETHYLENE BAG 20*20', 'kg', 157.90),
  ('SKU 32', 'POLYETHYLENE BAG 20*22', 'kg', 90.80),
  ('SKU 33', 'POLYETHYLENE BAG 24*24', 'kg', 23.72),
  ('SKU 34', 'POLYETHYLENE BAG 26*26', 'kg', 197.74),
  ('SKU 35', 'POLYETHYLENE BAG 28*28', 'kg', 96.80),
  ('SKU 36', 'POLYETHYLENE BAG 32*32', 'kg', 125.70),
  ...
  ('SKU 43', 'SANDPAPER COARSE 297', 'PCS', 26.00),
  ('SKU 45', 'EMERY PAPER J-297 FINE', 'PCS', 0.00),
  ('SKU 46', 'RANI PAPER', 'PCS', 144.00),
  ('SKU 47', 'EMERY PAPER 320 NO', 'PCS', 256.00),
  ('SKU 48', 'GREEN POLISHING BAR', 'PCS', 75.00),
  ('SKU 49', 'MADRAS BUFF 12 * 12', 'PCS', 6.00),
  ('SKU 50', 'COTTON HAND GLOVES JE', 'PCS', 48.00),
  ('SKU 51', 'KNITTED HAND GLOVES SMALL', 'PCS', 141.00),
  ('SKU 52', 'KNITTED HAND GLOVES HEAVY', 'PCS', 0.00),
  ('SKU 53', 'PVC-COATED HAND GLOVES', 'PCS', 21.00),
  ('SKU 54', 'GREEN SCOTCH-BRITE HAND PAD', 'PCS', 186.00),
  ...
  ('SKU 59', 'WOODEN STICK', 'PCS', 16.00),
  ('SKU 60', 'ALUMINUM BRAZING POWDER (PUNE)', 'PCS', 6.00),
  ('SKU 61', 'ALUMINUM BRAZING POWDER (MUMBAI)', 'PCS', 7.00),
  ('SKU 62', 'BOPP TAPE ROLL (BROWN)', 'PCS', 329.00),
  ...
  ('SKU 79', 'WHITE TAPE ROLL 3"', 'PCS', 335.00),
  ('SKU 80', 'WHITE TAPE ROLL 1/2"', 'PCS', 1024.00),
  ('SKU 81', 'WOOD BLOCK', 'PCS', 9.00),
  ('SKU 82', 'SAW FIREWOOD', 'KGS', 2800.00),
  ('SKU 83', 'GLASS WOOD', 'KG', 3.60),
  ('SKU 84', 'MODERN M.JUG BAKLITE KNOB', 'PCS', 380.00),
  ('SKU 85', '6KW ELECTRIC HEATER', 'KGS', 3.00),
  ('SKU 86', 'CONTAINER CARGO NET', 'PCS', 12.00);
...
SELECT
  ...
  CONCAT('Physical count: ', t.closing + 0, ' (system was ', i.current_stock + 0,
         ...
         '). Synced from IMS (Stores) sheet Closing Stock for 2026-09-14.'),
  'Active',
  'Akhilesh Vyas',
  ''
FROM   tmp_stores_closing_20260914 t
...
-- ----------------------------------------------------------------------------
UPDATE ims_items i
  JOIN tmp_stores_closing_20260914 t ON t.item_code = i.item_code
...
-- STEP 6 -- verification.
-- ----------------------------------------------------------------------------
-- Expect: stores_items >= 85, sum_stock = 84849.1 + stock of the
-- 2 blank-closing item(s) left untouched.
...
FROM   ims_transactions
WHERE  direction = 'ADJ' AND txn_date = '2026-09-14' AND created_by = 'Akhilesh Vyas'
ORDER  BY id;
