...
--
-- Source : Google Sheet "IMS" tab (gid=1105757494) of
--          https://docs.google.com/spreadsheets/d/1hCHEXDFXQtKqK6HrJSPA5foWjxmTW8WdUyroZd0QLno
--          Column A = Item Code, column L = "Closing Stock". Read 2026-08-11.
--
...
--
-- WHY THIS MATTERS: the original ims_items_trading_import.sql computed stock as
-- sum(IN) - sum(OUT) from a sheet whose IN rows were all logged as 0, so it left
-- 275 items NEGATIVE (TRD580 sits at -917 where this sheet says 917). This update
-- replaces those computed balances with the sheet's own counted closing figure.
--
-- WHAT'S IN HERE
--   - 552 items with a usable closing stock (of 580 rows in the sheet).
--   - 333 positive, 217 zero, 2 negative (TRD197=-0.35, TRD373=-0.10).
--     Six more items were negative only by floating-point noise (~-1e-14) and
--     round to 0.00 at DECIMAL(12,2), which is what the DB column stores.
--   - 28 sheet rows have a BLANK closing stock and are deliberately NOT
...
--   - The sheet's "Max Level" (column E) is NOT touched — you asked for closing
--     stock only.
--
...
CREATE TABLE ims_items_trading_stock_bak_20260811 AS
SELECT item_code, current_stock AS old_stock, NOW() AS backed_up_at
FROM   ims_items
WHERE  category = 'Trading';

SELECT COUNT(*) AS backed_up_rows FROM ims_items_trading_stock_bak_20260811;
...
INSERT INTO trd_closing_import (item_code, description, size, uom, closing_stock) VALUES
  ...
  ('TRD6', 'Alum. Bar 10221 x 6061T6 x 2000', '152.40,', 'KG', 15.00),
...
-- ── STEP 5 — VERIFY ─────────────────────────────────────────────────────────
-- Every row should show matched='YES'.
...
-- Keep ims_items_trading_stock_bak_20260811 for a while — it is the only undo.


-- ── ROLLBACK (if the numbers turn out wrong later) ──────────────────────────
-- UPDATE ims_items i
-- JOIN   ims_items_trading_stock_bak_20260811 b ON b.item_code = i.item_code
